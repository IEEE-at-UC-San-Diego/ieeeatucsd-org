...
- A local self-hosted **Convex** backend runs in Docker. On a fresh VM: start the daemon with `sudo dockerd` (systemd is unavailable; run it in a background/tmux session), then `bun run dev:convex` (needs `tools/convex-local/.env`, which can just set `PORT=3210`/`SITE_PROXY_PORT=3211`/`DASHBOARD_PORT=6791`). Backend: `http://127.0.0.1:3210`; Convex dashboard UI: `http://127.0.0.1:6791`.
  ...
  - Several env vars must be set **on the Convex deployment** (`bunx convex env set ...`), not just in the app `.env`: `LOGTO_ENDPOINT` + `LOGTO_APP_ID` (needed at deploy time by `convex/auth.config.ts`) and `CONVEX_SESSION_SECRET` (must match the dashboard's value — `permissions.ts` verifies the legacy session token inside Convex, so a missing/mismatched value makes authenticated queries like `users:getMe` fail with "Missing CONVEX_SESSION_SECRET").
