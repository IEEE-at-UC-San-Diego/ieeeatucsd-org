...
export const useConstitutionData = () => {
        ...
        const initializeConstitution = async () => {
            ...
            try {
                ...
                const unsubscribeSession = onSnapshot(sessionRef, (doc) => {
                    if (doc.exists()) {
                        const session = { id: doc.id, ...doc.data() } as ConstitutionCollaborationSession;
    ...
    const updateUserPresence = async (selectedSection?: string | null) => {
        ...
        const updatedUsers = currentActiveUsers.filter(u => u.userId !== user.uid);
    ...
    useEffect(() => {
        return () => {
            if (user && collaborationSession) {
                const sessionRef = doc(db, 'collaborationSessions', constitutionId);
                const updatedUsers = (collaborationSession.activeUsers || [])
    ...
    const addSection = async (type: ConstitutionSection['type'], parentId?: string) => {
        if (!user) return;
        ...
        const existingArticles = sections.filter(s => s.type === 'article').length;
        const existingAmendments = sections.filter(s => s.type === 'amendment').length;
        ...
        switch (type) {
            ...
            case 'section':
                if (parentId) {
                    const parentSections = sections.filter(s => s.parentId === parentId && s.type === 'section').length;
    ...
    const updateSection = async (sectionId: string, updates: Partial<ConstitutionSection>) => {
        if (!user) return;
