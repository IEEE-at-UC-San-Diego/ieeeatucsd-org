...
export class PreviewPdfValidator {
    ...
    constructor(constitution: Constitution | null, sections: ConstitutionSection[]) {
    ...
    private validateSectionStructure(result: ValidationResult): void {
        ...
        articles.forEach((article, index) => {
            if (!article.order && article.order !== 0) {
                result.warnings.push(`Article "${article.title || 'Untitled'}" is missing order property`);
            }
        });

        amendments.forEach((amendment, index) => {
            if (!amendment.order && amendment.order !== 0) {
                result.warnings.push(`Amendment "${amendment.title || 'Untitled'}" is missing order property`);
            }
        });
        ...
        this.sections.forEach(section => {
            if (!section.content || section.content.trim() === '') {
                result.warnings.push(`Section "${section.title || 'Untitled'}" has no content`);
            }
        });
    }
        ...
        this.sections.forEach(section => {
            if (section.content) {
                ...
                if (section.content.includes('\t')) {
                    result.warnings.push(`Section "${section.title}" contains tab characters that may render inconsistently`);
                }

                // Check for excessive line breaks
                if (section.content.includes('\n\n\n')) {
                    result.suggestions.push(`Section "${section.title}" has multiple consecutive line breaks - consider using double line breaks only`);
                }
                ...
                if (imageMatches) {
                    imageMatches.forEach(match => {
                        if (match === '[IMAGE:]') {
                            result.warnings.push(`Section "${section.title}" has empty image placeholder`);
                        }
                    });
                }
            }
    ...
    private validateTypography(result: ValidationResult): void {
        ...
        articles.forEach(article => {
            if (article.title && article.title !== article.title.trim()) {
                result.warnings.push(`Article "${article.title}" has leading/trailing whitespace`);
            }
        });

        amendments.forEach(amendment => {
            if (amendment.title && amendment.title !== amendment.title.trim()) {
                result.warnings.push(`Amendment "${amendment.title}" has leading/trailing whitespace`);
            }
        });
    }
                ...
                if (imageMatches) {
                    imageMatches.forEach(match => {
                        ...
                        if (description.length > 100) {
                            result.suggestions.push(`Image description in "${section.title}" is very long and may affect layout`);
                        }
                    });
                }
            }
    ...
    generateReport(): string {
        ...
        if (validation.isValid) {
            report += '✅ VALIDATION PASSED - Preview and PDF should be consistent\n\n';
        } else {
            report += '❌ VALIDATION FAILED - Issues found that may cause inconsistencies\n\n';
        }
