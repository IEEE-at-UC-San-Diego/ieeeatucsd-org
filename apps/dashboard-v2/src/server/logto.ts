...
function getConfig() {
	...
	if (!endpoint) {
		throw new Error(
			"Missing required env: LOGTO_ENDPOINT (or VITE_LOGTO_ENDPOINT)",
		);
	}
	if (!appId) {
...
async function getM2MToken(): Promise<string> {
	...
	if (!response.ok) {
		...
		throw new Error(
			`Failed to get Logto M2M token: ${response.status} ${text}`,
		);
	}
...
export async function findLogtoUserByEmail(
	email: string,
	...
	if (!response.ok) {
		const text = await response.text();
		throw new Error(
			`Failed to find Logto user by email: ${response.status} ${text}`,
		);
	}
...
export async function ensureLogtoRoles() {
	...
	if (!listResponse.ok) {
		...
		throw new Error(
			`Failed to list Logto roles: ${listResponse.status} ${text}`,
		);
	}

	const existingRoles = (await listResponse.json()) as Array<{
		id: string;
		name: string;
	}>;
	...
	for (const roleName of ROLE_NAMES) {
		...
		if (!createResponse.ok) {
			...
			throw new Error(
				`Failed to create Logto role ${roleName}: ${createResponse.status} ${text}`,
			);
		}
...
export async function ensureRoleOnLogtoUser(
	logtoUserId: string,
	role: AppRole,
) {
...
export async function syncOfficerRolesOnLogtoUser(
	logtoUserId: string,
	newRole: AppRole,
) {
	await syncAppRoleOnLogtoUser(logtoUserId, newRole);
}

export async function syncAppRoleOnLogtoUser(
	logtoUserId: string,
	newRole: AppRole,
) {
	const roleMap = await ensureLogtoRoles();
	...
	const rolesToRemove = ROLE_NAMES.filter((r) => r !== newRole);
