...
async function handle({ request }: { request: Request }) {
	try {
		...
		if (
			!isPrivileged &&
			requesterUsername &&
			normalizedUsername !== requesterUsername
		) {
			return new Response(
				JSON.stringify({
					success: false,
					error: "Forbidden: cannot create another user's mailbox",
				}),
		...
		if (result.success && result.ieeeEmail) {
			try {
				...
				if (convexUrl) {
					...
					const fn =
						"users:setIEEEEmail" as unknown as FunctionReference<"mutation">;
					...
					await client.mutation(fn, {
						logtoId,
						authToken: token,
						ieeeEmail: result.ieeeEmail,
					});
				}
			} catch (convexError) {
				console.error(
					"Failed to update Convex user with IEEE email:",
					convexError,
				);
