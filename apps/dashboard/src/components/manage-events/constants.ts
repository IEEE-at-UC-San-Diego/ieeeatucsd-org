...
export function normalizeDepartment(
	value?: string,
): EventDepartmentValue | undefined {
...
export function formatEventTypeLabel(value?: string): string {
	...
	return (
		EVENT_TYPE_OPTIONS.find((option) => option.value === normalized)?.label ||
		"Other"
	);
}
...
export function formatDepartmentLabel(value?: string): string {
	...
	return (
		DEPARTMENT_OPTIONS.find((option) => option.value === normalized)?.label ||
		"Unspecified"
	);
}
