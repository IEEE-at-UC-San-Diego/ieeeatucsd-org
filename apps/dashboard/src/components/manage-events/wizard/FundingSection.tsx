	...
	const handleAddInvoiceViaUpload = async (file: File) => {
		...
		if (shouldParseInvoiceWithAI(aiEnabled)) {
			try {
				...
				setUploadError(
					"AI parsing failed. Please enter invoice details manually.",
				);
			} catch {
				setUploadError(
					"AI parsing failed. Please enter invoice details manually.",
								...
								>
									...
									{invoice.invoiceFile && (
										<div className="flex items-center gap-2 text-xs text-green-600 bg-green-50 border border-green-200 rounded-lg px-3 py-2">
											...
											<span>
												{aiEnabled
													? "Invoice file attached (AI-scanned)"
													...
													>
														<div className="col-span-5">
															...
															<Input
																...
																onChange={(e) =>
																	updateLineItem(invoice._id, idx, {
									...
									<div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 border-t">
										<div className="space-y-1">
											<Label htmlFor={`tax-${invoice._id}`} className="text-xs">
												Tax ($)
											</Label>
											<Input
												id={`tax-${invoice._id}`}
										...
										<div className="space-y-1">
											<Label htmlFor={`tip-${invoice._id}`} className="text-xs">
												Tip ($)
											</Label>
											<Input
												id={`tip-${invoice._id}`}
