...
export default function FundRequestActionModal({
	...
	const [action, setAction] = useState<"approve" | "deny" | "needs_info">(
		"approve",
	);
	const [notes, setNotes] = useState("");
	const [fundingSource, setFundingSource] =
		useState<FUNDING_SOURCE>("department");
	const [isSubmitting, setIsSubmitting] = useState(false);

	const fundRequest = useAuthedQuery(
		api.fundRequests.get,
		logtoId && requestId ? { logtoId, id: requestId as any } : "skip",
	);
	...
	const handleSubmit = async () => {
		...
		try {
			...
			await updateStatus({
				...
				status: newStatus,
				reviewNotes:
					action === "approve" ? notes.trim() || undefined : undefined,
				infoRequestNotes: action === "needs_info" ? notes.trim() : undefined,
			});

			toast.success(
				`Request ${action === "needs_info" ? "returned for info" : action === "approve" ? "approved" : "denied"}`,
			);
			onClose();
		} catch (error: any) {
			...
			<DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
				<DialogHeader>
					<div className="space-y-1">
						...
						<div className="flex items-center gap-2 text-sm text-muted-foreground font-normal">
							...
							<span>•</span>
							<span>
								Submitted on {formatDate(fundRequest.createdAt || Date.now())}
							</span>
						</div>
					</div>
				</DialogHeader>
				...
				<div className="flex flex-col md:flex-row gap-6 min-h-0">
					...
					<div className="flex-1 space-y-4 overflow-y-auto pr-2">
						...
						<div className="flex items-center justify-between p-4 rounded-lg border bg-muted/50">
							<div>
								...
								<p className="font-medium">
									{fundRequest.submittedByName || "Unknown"}
								</p>
								<p className="text-xs text-muted-foreground">
									{fundRequest.submittedByEmail}
								</p>
							</div>
						...
						<div className="space-y-2">
							...
							<div className="flex gap-2">
								<Badge variant="outline" className="capitalize">
									{DEPARTMENT_LABELS[fundRequest.department] ||
						...
						{/* Attachments & Links */}
						{(fundRequest.vendorLinks?.length ||
							fundRequest.attachments?.length) && (
							<div className="space-y-4">
								{fundRequest.vendorLinks &&
									fundRequest.vendorLinks.length > 0 && (
										<div>
								...
								{fundRequest.attachments &&
									fundRequest.attachments.length > 0 && (
										<div>
											<h4 className="text-xs font-bold text-muted-foreground uppercase mb-2">
												Attachments
											</h4>
