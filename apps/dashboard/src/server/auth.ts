...
export async function requireApiAuth(
	...
	options?: {
		...
		validateAccessToken?: (
			accessToken: string,
		) => Promise<LogtoOidcMeResponse | null>;
		validateProvisionedUser?: (
			logtoId: string,
		) => Promise<
			| ({ role?: UserRole; aiFeaturesEnabled?: boolean } & Record<
					string,
					unknown
			  >)
			| null
	...
	| {
			...
			user: { role?: UserRole; aiFeaturesEnabled?: boolean } & Record<
				string,
				unknown
			>;
	  }
...
> {
	...
	const validateAccessToken =
		options?.validateAccessToken ?? validateLogtoAccessToken;
	const validateProvisionedUser =
		options?.validateProvisionedUser ?? validateLogtoId;
	...
	if (!user && !options?.allowUnprovisionedUser) {
		return new Response(
			JSON.stringify({
				error: "Authenticated user is not provisioned in dashboard",
			}),
			{ status: 401, headers: { "Content-Type": "application/json" } },
		);
	}
	...
	return {
		...
		body,
		claims,
		user: (user ?? {}) as { aiFeaturesEnabled?: boolean } & Record<
			string,
			unknown
		>,
	};
