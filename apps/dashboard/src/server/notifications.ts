...
export async function sendReimbursementSubmitted(
	data: ReimbursementSubmittedData,
) {
	const details: DetailRow[] = [
		{ label: "Title", value: escapeHtml(data.title) },
		{
			label: "Amount",
			value: `<strong style="color:#059669;">${formatCurrency(data.totalAmount)}</strong>`,
		},
		{
			label: "Department",
			value: escapeHtml(
		...
		{
			label: "Submitted By",
			value: `${escapeHtml(data.submitterName)} (${escapeHtml(data.submitterEmail)})`,
		},
	];
	...
	if (data.additionalInfo) {
		details.push({
			...
			value: escapeHtml(data.additionalInfo),
		});
	}
	...
	const treasurerHtml = renderEmail({
		...
		ctaButton: {
			text: "Review Request",
			url: `${DASHBOARD_BASE}/manage-reimbursements`,
		},
	...
	const userHtml = renderEmail({
		...
		ctaButton: {
			...
			url: `${DASHBOARD_BASE}/reimbursement`,
		},
		referenceId: data.reimbursementId,
		contactEmail: TREASURER_EMAIL,
		footerText: "IEEE at UC San Diego — Reimbursement System",
...
export async function sendReimbursementStatusChanged(
	data: ReimbursementStatusChangedData,
) {
	...
	if (data.previousStatus) {
		details.push({
			...
			value: escapeHtml(statusLabel(data.previousStatus)),
		});
	}
	if (data.changedByName) {
		details.push({
			label: "Updated By",
			value: escapeHtml(data.changedByName),
		});
	}
	...
	if (data.rejectionReason) {
		notes.push({
			title: "Reason",
			content: data.rejectionReason,
			variant: "danger",
		});
	}
	if (data.paymentDetails) {
		const parts: string[] = [];
		if (data.paymentDetails.confirmationNumber)
			parts.push(`Confirmation: ${data.paymentDetails.confirmationNumber}`);
		if (data.paymentDetails.amountPaid)
			parts.push(
				`Amount Paid: ${formatCurrency(data.paymentDetails.amountPaid)}`,
			);
		if (data.paymentDetails.paymentDate)
			parts.push(
				`Payment Date: ${formatDateShort(data.paymentDetails.paymentDate)}`,
			);
		if (parts.length) {
			notes.push({
...
export async function sendFundRequestSubmitted(data: FundRequestSubmittedData) {
	...
	const details: DetailRow[] = [
		...
		{
			label: "Amount",
			value: `<strong style="color:#059669;">${formatCurrency(data.amount)}</strong>`,
		},
		{
			label: "Category",
			value: escapeHtml(categoryLabels[data.category] || data.category),
		},
		{
			label: "Department",
			value: escapeHtml(
	...
	if (data.vendorLinksCount) {
		details.push({
			label: "Vendor Links",
			value: `${data.vendorLinksCount} link(s)`,
		});
	}
	if (data.attachmentsCount) {
		details.push({
			label: "Attachments",
			value: `${data.attachmentsCount} file(s)`,
		});
	}
	...
	const treasurerHtml = renderEmail({
		...
		ctaButton: {
			text: "Review Request",
			url: `${DASHBOARD_BASE}/manage-fund-requests`,
		},
	...
	const userHtml = renderEmail({
		...
		ctaButton: {
			text: "View Your Request",
			url: `${DASHBOARD_BASE}/fund-requests`,
		},
		referenceId: data.requestId,
		contactEmail: TREASURER_EMAIL,
		footerText: "IEEE at UC San Diego — Fund Request System",
...
export async function sendFundRequestStatusChanged(
	data: FundRequestStatusChangedData,
) {
	const variant = statusToVariant(data.newStatus);
	const label = statusLabel(data.newStatus);
	...
	if (data.selectedFundingSource) {
		details.push({
			...
			value: escapeHtml(
				fundingSourceLabels[data.selectedFundingSource] ||
					data.selectedFundingSource,
			),
		});
	}
	if (data.reviewerName) {
		details.push({
			label: "Reviewed By",
			value: escapeHtml(data.reviewerName),
		});
	}
	...
	if (data.reviewNotes) {
		notes.push({
			title: "Review Notes",
			content: data.reviewNotes,
			variant: data.newStatus === "denied" ? "danger" : "neutral",
		});
	}
	if (data.infoRequestNotes) {
		notes.push({
			title: "Information Requested",
			content: data.infoRequestNotes,
			variant: "warning",
		});
	}
...
export async function sendEventRequestSubmitted(
	data: EventRequestSubmittedData,
) {
	const details: DetailRow[] = [
	...
	if (data.expectedAttendance) {
		details.push({
			...
			value: String(data.expectedAttendance),
		});
	}
	if (data.department) {
		details.push({
	...
	const eventsHtml = renderEmail({
		...
		ctaButton: {
			text: "Review Request",
			url: `${DASHBOARD_BASE}/manage-events`,
		},
...
export async function sendEventRequestStatusChanged(
	data: EventRequestStatusChangedData,
) {
	const variant = statusToVariant(data.newStatus);
	const label = statusLabel(data.newStatus);
...
export async function sendEventRequestEdited(data: EventRequestEditedData) {
	const details: DetailRow[] = [
		...
		{
			label: "Edited By",
			value: `${escapeHtml(data.editorName)} (${escapeHtml(data.editorEmail)})`,
		},
	];
...
export async function sendAuditRequest(data: AuditRequestData) {
	const details: DetailRow[] = [
		...
		{
			label: "Amount",
			value: `<strong>${formatCurrency(data.totalAmount)}</strong>`,
		},
		{
			label: "Department",
			value: escapeHtml(
